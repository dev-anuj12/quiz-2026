/**
 * Standalone QRCode generator for KDK INDUCTION QUIZ 2026
 */
var QRCode;
(function () {
  ...
  QRPolynomial.prototype = {
    ...
    mod: function (e) {
      ...
      for (var j = 0; j < e.getLength(); j++) {
        num[j] ^= QRMath.gmult(e.get(j), QRMath.gexp(ratio));
      }
  ...
  QRCodeModel.prototype = {
    ...
    setupTypeInfo: function (test, maskPattern) {
      ...
      for (var j = 0; j < 15; j++) {
        ...
        if (j < 8) this.modules[8][this.moduleCount - j - 1] = mod;
        else if (j < 9) this.modules[8][15 - j - 1 + 1] = mod;
        else this.modules[8][15 - j - 1] = mod;
      }
  ...
  QRCode.prototype.makeCode = function (text) {
    ...
    var colorDark = this._htOption.colorDark || "#0c0e13";
    var colorLight = this._htOption.colorLight || "#ffffff";
    var correctLevel = this._htOption.correctLevel !== undefined ? this._htOption.correctLevel : QRCode.CorrectLevel.M;

    var qr;
    for (var t = 1; t <= 12; t++) {
      try {
        qr = new QRCodeModel(t, correctLevel);
        qr.addData(text);
        qr.make();
        break;
      } catch (e) {
    ...
    if (!qr) {
      qr = new QRCodeModel(14, QRCode.CorrectLevel.L);
      qr.addData(text);
      qr.make();
    }
    ...
    canvas.setAttribute("aria-label", "QR Code: " + text);
